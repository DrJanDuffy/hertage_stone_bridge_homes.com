	...
	component$<RealScoutOfficeListingsWidgetProps>(
		({
			...
			useVisibleTask$(() => {
				...
				console.log("📜 RealScout Debug: Script element found:", !!scriptElement);
				if (scriptElement) {
					console.log("📜 RealScout Debug: Script src:", scriptElement.getAttribute("src"));
				}
				...
				console.log("🧩 RealScout Debug: Custom element definition:", customElement);
				
				if (customElement) {
					console.log("✅ RealScout Debug: Custom element already defined, widget ready");
					isScriptLoaded.value = true;
					return;
				}

				console.log("⏳ RealScout Debug: Custom element not yet defined, waiting...");

				// Set up timeout fallback (10 seconds)
				const timeoutId = setTimeout(() => {
					console.error("⏰ RealScout Debug: Timeout reached - widget failed to load within 10 seconds");
					hasError.value = true;
					errorMessage.value = "RealScout widget failed to load. Please refresh the page or contact support.";
				}, 10000);
				...
				customElements
					...
					.then(() => {
						console.log("✅ RealScout Debug: Custom element successfully defined");
						clearTimeout(timeoutId);
						isScriptLoaded.value = true;
					})
					.catch((error) => {
						// 3) Log any errors in the whenDefined promise
						console.error("❌ RealScout Debug: Error waiting for custom element:", error);
				...
				realScoutElements.forEach(el => {
					console.log(`🔍 RealScout Debug: ${el} defined:`, !!customElements.get(el));
				});
				...
				<div class="min-h-[400px] bg-gray-50 rounded-lg p-8">
					{hasError.value ? (
						...
						<div class="text-center text-gray-500 mb-4">
							...
							<div class="text-xs text-gray-400 mt-2">Check browser console for debug info</div>
						</div>
					) : (
						<realscout-office-listings
