...
export const RealScoutWidget = component$<RealScoutWidgetProps>(
	({ agentEncodedId, widgetType = "search", className = "" }) => {
					...
					if (window.RealScout) {
						const widgetConfig = {
							...
							container: document.getElementById(
								`realscout-widget-${agentEncodedId}`,
							),
							theme: "modern",
							responsive: true,
							showPoweredBy: true,
		...
		if (hasError.value) {
			return (
				<div
					class={`bg-red-50 border border-red-200 rounded-lg p-6 text-center ${className}`}
				>
					...
					<p class="text-red-700 mb-4">
						{errorMessage.value ||
							"Unable to load the RealScout search widget at this time."}
					</p>
					<a
		...
		return (
			<div
				class={`bg-white rounded-lg shadow-lg border border-gray-200 min-h-[480px] ${className}`}
			>
				{!isLoaded.value ? (
